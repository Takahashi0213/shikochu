#pragma once
class Sekuteimu : public IGameObject
{
public:
	Sekuteimu();
	~Sekuteimu();
	...
	//ポジションを返す関数
	CVector3 Sekuteimu::Getm_Position() {
		return m_position;
	}
	//ステートを返す関数
	int Sekuteimu::GetEState() {
		return m_stete;
	}
	//自分が死ぬ関数
	int Sekuteimu::SetDeath() {
		m_stete = Estete_Death;
		return 0;
	}
	//自分が死ぬ関数2
	void Sekuteimu::SetDeath2() {
...
private:
	...
	//攻撃関連
	int timecount = 0;
	const float waittimer = 10.0f;
	const float attacktime = 120.0f;
