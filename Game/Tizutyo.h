#pragma once
class Tizutyo : public IGameObject
{
public:
	Tizutyo();
	~Tizutyo();
	...
	//ポジションを返す関数
	CVector3 Tizutyo::Getm_Position() {
		return m_position;
	}
	//ステートを返す関数
	int Tizutyo::GetEState() {
		return m_stete;
	}
	//自分が死ぬ関数
	int Tizutyo::SetDeath() {
		m_stete = Estete_Death;
		return 0;
	}

	//自分が死ぬ関数2
	void Tizutyo::SetDeath2() {
...
private:
	...
	const int move_starttimer = 120; //再移動するタイマー
									//予備動作関連
